...
export function ProjectsSection({ projects }: ProjectsSectionProps) {
  ...
  return (
    <section id="works" className="w-full py-16 sm:py-20 bg-white text-black scroll-mt-14 sm:scroll-mt-16">
      <div className="w-full px-4 sm:px-6 lg:px-12 max-w-7xl mx-auto">
        ...
        <Reveal>
          <div className="flex items-start gap-4 sm:gap-6 mb-8 sm:mb-12 pb-4 sm:pb-6 border-b-2 border-black">
            <span className="text-mono text-[#737373] mt-1 text-xs sm:text-sm">[03]</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {rest.map((project, i) => (
              <Reveal key={project.id} delayMs={i * 60}>
                <Link
                  href={`/projects/${project.slug}`}
                  className="group border border-[#e5e5e5] p-6 bg-white hover:bg-black hover:border-black transition-colors duration-250 flex flex-col justify-between focus-visible:outline focus-visible:outline-2 focus-visible:outline-black h-full"
                >
                  <div>
                    {project.thumbnail_url && (
