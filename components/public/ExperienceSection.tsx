...
export function ExperienceSection({ experiences }: ExperienceSectionProps) {
  ...
  return (
    <section id="experience" className="w-full py-16 sm:py-20 px-4 sm:px-6 lg:px-12 max-w-7xl mx-auto scroll-mt-14 sm:scroll-mt-16">
      {/* Section header */}
      <Reveal>
        <div className="flex items-start gap-4 sm:gap-6 mb-8 sm:mb-12 pb-4 sm:pb-6 border-b-2 border-foreground">
          <span className="text-mono text-[#737373] mt-1 text-xs sm:text-sm">[04]</span>
          ...
          <Reveal key={exp.id} delayMs={i * 80}>
            ...
            >
              ...
              <div className="shrink-0">
                <p className="text-mono text-[#737373] mb-1 uppercase">{TYPE_LABELS[exp.type] || exp.type}</p>
                  ...
                  <ul className="space-y-1 mb-4">
                    {exp.responsibilities.map((resp) => (
                      <li key={resp.id} className="flex items-start gap-2 text-sm text-[#525252]">
                        <span className="text-mono text-[#737373] shrink-0 mt-0.5">—</span>
                        {resp.text}
                      </li>
                    ))}
                  </ul>
